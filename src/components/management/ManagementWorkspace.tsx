...
function ManagementDataWorkspace({ onAskContextChange, section }: { onAskContextChange?: (context: CfsAiSearchRequest["filter_context"]) => void; section: ManagementSection }) {
  ...
  const openPlanningBuilder = (
    ...
    ...context,
    analysisPeriod: context.analysisPeriod ?? period,
    fitExtent: context.fitExtent ?? (context.selectedHotspotId || context.selectedSignalId ? "selection" : "results"),
    sourceManagementPage: section,
...
function Overview({ development, economics, flood, hotspots, hotspotRows, model, schools, trendRows, trendSource, openEconomicsBuilder, openManagementDetail, openPlanningBuilder, period }: any) {
  ...
  return <>
    ...
    <ThreeColumns>
      <Panel eyebrow="Reference context" info={insight("constraintPosture", combineTrust("Planning constraints", [floodData, schoolData]))} title="Review context"><StatusRows rows={[["Flood review", metric(flood.metrics, "review-required-parcels")], ["High/severe impact", metric(flood.metrics, "high-severe-buildability")], ["School assignment review", metric(schools.metrics, "assignment-review")]]} /><Action onClick={() => openPlanningBuilder({ activeLayerIds: ["flood-risk", "fema-flood-zones", "school-pressure"], filter: { population: "flood_review" }, planningMode: "countywide", selectionType: "parcel-population", selectionValue: "flood_review", sourceInsightType: "overview-constraints" })}>Open in Analyst View</Action></Panel>
...
function Planning({ development, flood, hotspots, hotspotMarkers, hotspotRows, schools, selected, setSelected, trendRows, trendSource, openBuilder, period }: any) {
  ...
  return <>
    <KpiGrid items={[
      { actionLabel: "Inspect permits", label: managementKpis.permitActivity.label, value: countValue(development.source, development.isLoading, development.totalPermits), status: period.label, info: insight("permitActivity", permits, managementKpis.permitActivity, period.label), onAction: () => openBuilder({ activeLayerIds: ["permit-activity"], filter: { population: "permit_activity" }, selectionType: "parcel-population", selectionValue: "permit_activity", sourceInsightType: "planning-constraints" }) },
      { actionLabel: "Inspect parcels", label: managementKpis.activeParcels.label, value: countValue(development.source, development.isLoading, development.activeParcelCount), status: period.label, info: insight("activeHotspots", permits, managementKpis.activeParcels, period.label), onAction: () => openBuilder({ activeLayerIds: ["permit-activity", "development-hotspots"], filter: { population: "active_development_parcels" }, selectionType: "parcel-population", selectionValue: "active_development_parcels", sourceInsightType: "planning-constraints" }) },
    ]} />
    <TwoColumns>
    ...
    <TwoColumns>
      <Panel eyebrow="Current FEMA and parcel context" focus="flood-review" info={insight("floodReview", floodData, managementKpis.floodReview, period.label)} title="Flood Review"><StatusRows rows={[["Parcels requiring review", metric(flood.metrics, "review-required-parcels")], ["High/severe flood impact", metric(flood.metrics, "high-severe-buildability")]]} /><div className="flex flex-wrap gap-2"><Action onClick={() => openBuilder({ activeLayerIds: ["flood-risk", "fema-flood-zones"], filter: { population: "flood_review" }, planningMode: "countywide", selectionType: "parcel-population", selectionValue: "flood_review", sourceInsightType: "planning-constraints" })}>Open in Analyst View</Action><Action onClick={() => openBuilder({ activeLayerIds: ["flood-risk", "fema-flood-zones"], filter: { population: "flood_high_severe" }, planningMode: "countywide", selectionType: "parcel-population", selectionValue: "flood_high_severe", sourceInsightType: "planning-constraints" })}>Inspect high / severe</Action></div></Panel>
...
function Economics({ development, economics, openBuilder, period, trendDirection, trendRows }: any) {
  ...
  return <>
    ...
    <Panel eyebrow="Scenario results" info={insight("scenarioResults", economicData)} title="Screening-level comparisons">
      ...
      <Action onClick={() => openBuilder("economic-insights", currentScenario?.scenario_id)}>Open in Analyst View</Action>
    </Panel>
  ...
  return <>
    ...
    <TwoColumns>
      ...
      <Panel eyebrow="Historical signal watchlist" info={insight("signalWatchlist", signalData)} title={selected ? selected.approximateAreaLabel || "Selected development signal" : "Highest-signal areas"}>
        ...
        <Action disabled={!selected} onClick={() => selected && openBuilder({ planningMode: "modelLab", selectedParcelId: selected.officialParcelId, selectedSignalContext: selected, selectedSignalId: selected.officialParcelId, sourceInsightType: "development-signal" })} testId="management-signal-builder-handoff">Investigate in Analyst View</Action>
      </Panel>
    </TwoColumns>
