...
function Overview({ development, economics, flood, hotspots, hotspotRows, model, schools, trendRows, trendSource, openEconomicsBuilder, openManagementDetail, openPlanningBuilder, period }: any) {
  ...
  return <>
    <KpiGrid items={[
      ...
      { actionLabel: "Inspect area", label: "Top Development Area", value: hotspotRows.at(0)?.label ?? "Unavailable", status: hotspotRows.at(0) ? `${number.format(hotspotRows[0].value)} permits · ${period.label}` : period.label, info: insight("planningAttention", hotspotData), onAction: () => { const marker = hotspots.markers?.[0]; marker && openPlanningBuilder({ planningMode: "countywide", selectedHotspotContext: toHotspotContext(marker), selectedHotspotId: marker.officialParcelId, selectedParcelId: marker.officialParcelId, selectionType: "hotspot", selectionValue: marker.officialParcelId, sourceInsightType: "planning-hotspot" }); } },
    ]} />
    <TwoColumns>
