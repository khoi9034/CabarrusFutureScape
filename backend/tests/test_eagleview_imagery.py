                    ...
                    "response": {
                        "images": {
                            "north": [
                                {
                                    "date": "2026-08-14",
                                    "imageResource": "https://images.example/north",
                                }
                            ],
                            "south": [],
                            "east": [
                                {
                                    "date": "2026-07-02",
                                    "imageResource": "https://images.example/east",
                                }
                            ],
                        }
                    }
...
def test_image_authentication_and_proxy_url(monkeypatch) -> None:
    client = FakeHttpClient(
        [
            ...
            httpx.Response(
                200,
                request=httpx.Request("GET", "https://images.example/test"),
    ...
    image = provider.fetch_image(
        ImageryRecord("west", "2026-08-14", "https://images.example/west"),
        width=800,
        height=600,
    )
    ...
    assert client.urls[1] == (
        "https://images.example/west/short-lived-token/"
...
def test_invalid_token_and_missing_configuration_are_safe() -> None:
    ...
    with pytest.raises(ImageryProviderUnavailable, match="did not return a token"):
        provider.fetch_image(
            ImageryRecord("north", None, "https://images.example/north"),
            width=800,
            height=600,
        )
...
class FakeProvider:
    ...
    def search(self, latitude: float, longitude: float) -> list[ImageryRecord]:
        ...
        if self.failure:
            raise self.failure
        return [
            ImageryRecord("north", "2026-08-14", "https://images.example/north"),
            ImageryRecord("west", None, "https://images.example/west"),
        ]
