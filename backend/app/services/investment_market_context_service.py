...
from app.connectors.census_acs import ACS_YEAR, CensusAcsConnector
...
def acs_status(db: Session) -> dict[str, Any]:
    ...
    return {
        ...
        "limitations": [SAFE_LIMITATION, "Margins of error are not yet surfaced in this compact context view."],
    }


def refresh_acs_market_context(db: Session, *, year: int = ACS_YEAR) -> dict[str, Any]:
    _ensure_tables(db)
...
def build_market_context(record: dict[str, Any], county_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "source": "U.S. Census Bureau ACS API",
        "acs_year": int(record.get("acs_year") or ACS_YEAR),
        ...
        "data_confidence": "Moderate" if record.get("total_population") is not None and total_households is not None else "Limited",
        ...
        "limitations": [
            SAFE_LIMITATION,
            "ACS values are aggregate tract estimates with margins of error; this compact view does not yet surface MOE values.",
...
def _resolve_parcel_tract_geoid(db: Session, parcel_id: str) -> str | None:
    ...
    point = db.execute(
        text(
            """
            SELECT
              ST_X(ST_PointOnSurface(geometry))::float8 AS longitude,
              ST_Y(ST_PointOnSurface(geometry))::float8 AS latitude
            FROM parcels_enriched
            WHERE official_parcel_id = :parcel_id
    ...
    if not point:
        return None
    try:
        # ponytail: cached geocoder lookup, replace with local tract overlay if candidate volume grows.
        geoid = CensusAcsConnector(timeout_seconds=5).tract_geoid_for_point(float(point["longitude"]), float(point["latitude"]))
    except Exception:
        return None
    if geoid:
        db.execute(
            text(
                f"""
                INSERT INTO {PARCEL_GEO_TABLE} (parcel_id, tract_geoid, source, resolved_at)
                VALUES (:parcel_id, :tract_geoid, 'census_geocoder', now())
                ON CONFLICT (parcel_id) DO UPDATE SET tract_geoid = EXCLUDED.tract_geoid, source = EXCLUDED.source, resolved_at = now()
                """
            ),
            {"parcel_id": parcel_id, "tract_geoid": geoid},
        )
    return geoid
...
def _unavailable(reason: str, *, geoid: str | None = None) -> dict[str, Any]:
    return {
        ...
        "source_attribution": "U.S. Census Bureau ACS 5-year estimates.",
        "last_refreshed": None,
        "limitations": [SAFE_LIMITATION, reason],
    }
