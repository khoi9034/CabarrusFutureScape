"""Ingest Cabarrus County Current Zoning Districts into local PostGIS.

This is a local-development overlay pilot for Cabarrus FutureScape (CFS).
It writes the ArcGIS REST zoning layer to public.zoning and does not connect
the frontend dashboard, APIs, additional layers, forecasting, or AI systems.
"""
...
ARCGIS_LAYER_URL = (
    ...
    "opendata/MapServer/35"
)
DEFAULT_TABLE_NAME = "zoning"
...
SUMMARY_OUTPUT = OUTPUT_DIR / "ingest_zoning_summary.json"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Cabarrus County Current Zoning Districts into PostGIS.",
    )
    parser.add_argument("--source-url", default=ARCGIS_LAYER_URL)
...
def configure_logging(log_level: str) -> Path:
    ...
    log_path = LOG_DIR / f"ingest_zoning_{timestamp}.log"
    logging.basicConfig(
...
def create_requests_session() -> Session:
    ...
    session.headers.update({"User-Agent": "CabarrusFutureScape-ZoningIngestion/0.1"})
    return session
...
def download_features_with_pagination(
    ...
    logging.info(
        "Downloading zoning features with pagination: pageSize=%s limit=%s",
        page_size,
        limit or "all",
    )
    ...
    while len(features) < target_count:
        ...
        if not page_features:
            logging.warning("No zoning features returned at offset=%s.", offset)
            break

        features.extend(page_features)
        offset += len(page_features)
        logging.info("Downloaded %s/%s zoning features", len(features), target_count)
...
def fetch_object_ids(
    ...
    if not object_ids:
        raise ArcGISRestError("ArcGIS REST source returned no zoning object IDs.")
...
def download_features_with_object_ids(
    ...
    logging.info("Downloading zoning features with object ID fallback.")
    ...
    for start in range(0, len(object_ids), page_size):
        ...
        logging.info("Downloaded %s/%s zoning features", len(features), len(object_ids))
...
def features_to_geodataframe(features: list[dict[str, Any]]) -> gpd.GeoDataFrame:
    if not features:
        raise ValueError("No zoning features were downloaded.")

    logging.info("Building GeoDataFrame from %s zoning features.", len(features))
    ...
    if null_geometry_count:
        logging.warning("Dropping %s zoning features with null geometries.", null_geometry_count)
        gdf = gdf[gdf.geometry.notna()].copy()

    if gdf.empty:
        raise ValueError("All downloaded zoning features had null geometries.")
...
def create_engine_from_env() -> Engine:
    ...
    if not password:
        raise RuntimeError(
            "CFS_POSTGRES_PASSWORD is not set. Export it before running zoning ingestion."
        )
...
def verify_database(engine: Engine) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "is listening on localhost:5433, database cfs_dev exists, PostGIS is "
            "enabled, and CFS_POSTGRES_PASSWORD is correct."
        ) from error
...
def write_to_postgis(
    ...
    logging.info(
        "Writing %s zoning features to PostGIS table %s.%s with if_exists=%s.",
        len(gdf),
        schema,
        table,
        if_exists,
...
def log_geodataframe_summary(gdf: gpd.GeoDataFrame) -> dict[str, Any]:
    ...
    summary = {
        ...
        "zoning_code_sample": sorted(
            str(value)
            for value in gdf.get("zoningcode", []).dropna().unique()[:20]
        )
        if "zoningcode" in gdf.columns
        else [],
    }
...
def write_summary_file(summary: dict[str, Any]) -> Path:
    ...
    logging.info("Wrote zoning ingestion summary: %s", SUMMARY_OUTPUT)
...
def main() -> int:
    ...
    logging.info("Starting CFS Current Zoning ingestion pilot.")
    ...
    try:
        ...
        logging.info("CFS Current Zoning ingestion pilot complete.")
        return 0
    except Exception:
        logging.exception("CFS Current Zoning ingestion failed.")
        return 1
